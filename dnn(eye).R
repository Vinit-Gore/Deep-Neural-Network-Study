wholedata <- read.csv("Dnn/data/eye.csv",header = TRUE)
#wholedata <- wholedata[3000:7000,]
sc = 1 # Start attribute Column number 
ec = 14 # End attribute Column number
...
nn <- neuralnet(f,train,hidden=c(10),linear.output=FALSE,lifesign = "full")
save("nn",file = "nnsave.RData")
#plot(nn)
...
print(cm)
cat("Accuracy =",((cm[1]+cm[4]) / (cm[1]+cm[2]+cm[3]+cm[4])) ,"\n")
